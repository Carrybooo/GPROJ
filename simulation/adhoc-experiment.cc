...
for ( int i=0; i < nWifis; i++){
    ...
    pos.Set("X", StringValue("ns3::UniformRandomVariable[Min="+std::to_string(maxX * i/nWifis - 1)+"|Max="+std::to_string(maxX * i/nWifis)+"]"));
    pos.Set("Y", StringValue("ns3::UniformRandomVariable[Min="+std::to_string(maxY * i/nWifis - 1)+"|Max="+std::to_string(maxY * i/nWifis)+"]"));
    ...
    AnimationInterface anim ("adhoc-exp.xml");
    // anim.SetConstantPosition (adhocNodes.Get (0), 3.0 , 3.0);
    // anim.SetConstantPosition (adhocNodes.Get (1), 1.0 , 2.0);
    // anim.SetConstantPosition (adhocNodes.Get (2), 7.0 , 7.0);
    // anim.SetConstantPosition (adhocNodes.Get (3), 11.0 , 11.0);
    Simulator::Run();
